...
**A1.** https://www.datawrapper.de/_/Y7S5x/
After looking at the data and comparing different chart types, I ended up choosing the grouped bar chart because it highlighted the stark difference between Cox's and Hogan's performances in their elections. This allowed me to come to the conclusion that Hogan performed much better in his 2018 election compared to Cox in the 2022 election. I omitted Saint Mary's data just because it didn't have any data on Hogan's percentage of votes, but even without it, the data makes it clear that Hogan performed better.

```{r}
md_gov_county <- read_csv("data/md_gov_county.csv")
md_gov_county_18 <- read_csv("data/md_gov_county_18.csv")
md_gov_county_18_22 <- md_gov_county %>% left_join(md_gov_county_18, by='County')
md_gov_county_18_22 <- md_gov_county_18_22 %>% 
  rename(Hogan_18 = Hogan) %>% 
  rename(Cox_22 = Cox) %>% 
  mutate(total_votes_18 = Hogan_18 + Jealous + Quinn + Schlakman + `Write-ins.y`) %>% 
  mutate(total_votes_22 = Cox_22 + Moore + Lashar + Wallace + Harding + Sefcik + `Write-ins.x`) %>% 
  mutate(pct_cox = Cox_22/total_votes_22*100) %>% 
  mutate(pct_hogan = Hogan_18/total_votes_18*100) %>% 
...
**A2.** https://www.datawrapper.de/_/Ev7V9/
I think one of the main stories that this data can provide is about the COVID-19 pandemic and its effects on the way people voted in elections. This is especially important considering all the issues surrounding mail-in ballots that occurred at the time.
...
**A3.** The Germantown area has the highest number of 911 calls. I want to know more about the populations and demographics of these zip codes and see if there are any patterns between them.
https://www.datawrapper.de/_/Bck34/

```{r}
montgomery_overdoses <- read_csv("data/montgomery_2022_overdoses.csv")
montgomery_total_calls <- montgomery_overdoses %>% 
  group_by(zip) %>% 
  summarise(
    total_calls = n()
  )
montgomery_call_pcts <- montgomery_total_calls %>% left_join(montgomery_overdoses, "zip") %>% 
  group_by(zip) %>% 
  summarise(
    total_calls = n()
  ) %>% 
  ...
  select(zip, total_calls, pct_calls)

write_csv(montgomery_call_pcts, "data/montgomery_call_pcts.csv")
