...
**A1.** The committee name that comes up as the one that spent the most is "Jain Ashwani Friends of."

```{r}
media_maryland_expenses <- maryland_expenses_dates %>% 
  filter(str_detect(expense_category, "Media") & payee_name != "NA")
write_csv(media_maryland_expenses, "data/media_maryland_expenses.csv")
cleaned_payee_names <- read_csv("data/cleaned_payee_names.csv") %>% 
...
**A2.** The top spender, Ashwani Jain, a Maryland governor candidate who ran to make the state government more transparent and representative, focusing on a "Relief, Recovery, and Reform" plan. The second is Maryland comptroller Peter Franchot and his gubernatorial running mate, Monique Anderson-Walker, both who want to make Maryland safer, especially in terms of health inequities during the pandemic. Franchot also takes the third spot. Next is CASA in Action, a PAC that aims to elect "progressive leaders supporting immigrants and communities of color," according to their website. Billy Boniface is the Hartford County executive with a focus on conservative tax policies and support for law enforcement. WPF, also known as Working Families Party, is a PAC aiming to build a multiracial party of working people.

The ads for Ashwani Jain were all under the same name, Jainforgov, and all the ads were taken down becuase they didn't follow advertising standards.
...
**A3.** Candidates from the gubernatorial election that are missing from the list of top spenders are Rushern L. Baker, Douglas F. Gansler, Ralph Jaffe, Wes Moore, Tom Perez and Jerome M. Segal. Rushern Baker, Wes Moore and Tom Perez all ran Facebook ads, according to Facebook's ad library. 

The biggest recipient of media expenses by Wes Moore's committees (Moore Wes for Maryland and Moore Miller for Maryland Slate) is SKDK, a public affairs and political consulting firm, with over $6 million, followed by Foglamp, which helps with finding support through digital strategy, with over $16,000.

```{r}
media_maryland_expenses %>% 
...
**A4.** The list for spending on online advertising differs greatly the list about Facebook spenders. However, CASA in Action appears in the top 5 for online advertising as well. However, it looks like it has only spent a small portion of its online advertising dollars on Facebook.

```{r}
online_advertising <- read_csv("data/cleaned_payee_names.csv") %>% 
  ...
  arrange(desc(total_amount))
cleaned_payee_names %>%  
  filter(str_detect(committee_name, "Ball  Calvin Team  The") | str_detect(committee_name, "MSEA's Fund For Children And Public Education PAC") | str_detect(committee_name, "House Democratic Caucus Committee  Maryland") | str_detect(committee_name, "Baron-Williams Slate for Maryland") | str_detect(committee_name, "Casa in Action PAC"))
...
**A5** From the data, we know what political candidates are spending the highest amount of money on Facebook, especially for online advertising. Additionally, we know what other advertisers candidates are also spending their money on and where they are focusing on gathering their support, shown through various PACs. A story that could come out of this would be one comparing candidates' spending on online advertising and how that correlates to their votes and success during gubernatorial elections.
