...
voters_by_county <- voters_by_county %>% mutate(pct_dem = DEM/TOTAL*100) %>% arrange(desc(pct_dem))
```
...
**Answer** The first row of data for Maryland is being shown as headers instead of as part of the dataset.
...
**Answer** The headers remain, but now they are just numbered in order.
...
**Answer** Clean the names of jurisdictions so they don't contain all the periods at the end.
