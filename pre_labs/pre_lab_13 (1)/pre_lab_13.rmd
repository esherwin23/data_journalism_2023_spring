...
**Answer** It outputs a dataframe with columns about the state, who they voted for, the percentage of the vote Trump received, and two PPE columns.
```{r}
...
**Answer** The code outputs a scatterplot with a trend line that shows a possible positive correlation between the two variables.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** Now, the plot has a trend line that is not as steep compared to before, possibly showing a weaker relationship.
```{r}
...
**Answer** The table outputted displayed the correlation coefficients for the relationships between pct_trump and businesses_receiving_ppe_pct, as well pct_trump and ppe_amount_per_employee.
...
**Answer** The model outputs various bits of information, such as a p-value, confidence intervals, correlation coefficient, etc.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The output is similar to the data above, but the p-value is higher than before, showing the correlation could be a bit more random.
```{r}
...
**Answer** The table provides four numerical values that describe the means for ppp amount per employeee and businesses receiving ppe grouped by states that voted for Clinton and Trump.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** It calculated the means like before, but it also calculates a t-test value and a p-value. In this case, the p-value is pretty high, so the relationship is not very significant.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The data provided by the output is more promising, given that the t-test score is higher and the p-value is lower.
```{r}
