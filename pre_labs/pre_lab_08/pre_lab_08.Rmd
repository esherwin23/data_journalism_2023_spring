...
**Answer** First, the code is grabbing data from baltco_911_calls and putting it into a new data frame called baltco_911_calls_by_month. Then, the mutate function creates a new column for the month, which is then transformed from a number into an abbreviation. It then groups the data by month and counts the number of calls for each month. Finally, it arranges the data in order of greatest number of calls to least.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** The code creates a bar chart using the code for calls per month from before.
...
**Answer** The new term, reorder, puts the columns in order from least to greatest.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The x and y axes switched, causing the columns to become horizontal after adding the code coord_flip()
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The gray background was removed after adding the code theme_minimal()
...
**Answer** The chart now has a steel-blue background after adding the code theme_economist()
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The chart now has a title, new axis labels and a source after adding the code labs using title, x, y and caption.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The code creates a dataframe with two columns: one for the month and one for the total number of calls in that month.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** After running the code, a line chart is created for total number of calls per month.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The x-axis now lists each week, making it difficult to see each specific tick mark.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Each week label is now at an angle. It is still a bit hard to read, but more legible compared to before.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
