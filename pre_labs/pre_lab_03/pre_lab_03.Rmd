...
**Answer** It says there are one or more parsing issues and to call 'problems()'
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No, there is no error.
...
**Answer**Vendor names are characters, the amount field is a double and the date column is a character.
...
**Answer** 547 zip codes do not have five characters. I think that the data was messed up, causing leading 0's to be taken off of some zip codes.
...
payment <- payments %>% 
  mutate(`Vendor Zip` = ifelse(str_length(`Vendor Zip`) == 4, paste0("0",`Vendor Zip`), `Vendor Zip`))
```
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer** Some years are listed as NA, one as L, and one random one just called "contract."
...
**Answer** There are two amounts that are less than $100. 
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer** It changes all the headers to be lowercase and replaces spaces with underscores.
...
**Answer** 2 duplicates are possible.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** There is now a new column with zip codes that are only 5 digits long, and the zip codes that had 9 digits before now only use the first 5 digits.
...
**Answer** The zipcode that got the most money from the Maryland Tourism Board is 21202, located in Baltimore Maryland, earning $8,464,101. 
