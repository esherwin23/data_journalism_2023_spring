...
**Answer** Within umd_courses, the code is finding all the classes from each term and giving a count for the number of classes within each of those terms
...
**Answer** There is a new line of code: arrange(desc(count_classes)). This puts the terms in order from highest to lowest based on the number of classes for that term.
...
**Answer** The output now includes a third column for departments, and now arranges the data, first, by terms in chronological order and then by department in alphabetical order. This was changed with the line of code: arrange(term, department)
...
Compared to the previous codeblock, what has changed and how did that affect the result? 
**Answer** There is now a new column that provides the total number of seats in each term. The data is now arranged from highest to lowest by total number of seats.
...
**Answer** There are two new columns showing the mean and median number of seats for classes. The top terms all happened within fall terms.
...
**Answer** There are new lines of code (min_seats and max_seats) that list the minimum and maximum number of seats for each course. Additionally, the data is now grouped by department instead of term.
...
1.  What departments offer the most classes?

2.  Is each department relatively consistent with the number of classes it offers across each term?

3.  Which terms/year offered the least amount of classes? Does it line up with the beginning of COVID?
