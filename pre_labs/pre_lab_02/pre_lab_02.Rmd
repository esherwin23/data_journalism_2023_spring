...
**Answer** We made a new variable so that the selection creates a new data set that we can refer back to if needed.
...
**Answer:** The date is now listed with the year first, and months and days are listed as double digits, with values less than ten having a zero at the front. Additionally, it is now categorized as a data instead of a character.
...
**Answer** Months only appear once. All the days in a month were grouped together so they are all associated with just one row.
...
**Answer** The code is adding a new column called total_votes so that it adds up the votes of cox, moore, lashar, wallace and write-ins. Additionally, its creating a new column to show the percentage of votes won by moore.
...
**Answer** The items in the column are now listed as percentages instead of decimals after multiplying by 100.
...
**Answer** It is not more useful than descending order because it just justifies that he gets less votes in smaller counties, which we already knew by ordering in descending order.
...
**Answer** str_capitalise does the same as str_to_upper as it makes all the letters uppercase. str_to_lower is similar in that it returns all letters as lowercase.
...
**Answer** The mutate statement uses case_when to detect the names of each state in each row and assign them as their abbreviated versions in another column named "state." If none of the states are detected, then the last line of code makes sure nothing changes.
...
**Answer** At first the answer does not make sense because it does not provide a lot of information for one to work off of, it just gives the state and then a total. However, if you look at the code, you can tell that the total is the total amount of expenses for each state.
...
1. What departments have classes that have 10 seats or less?

2. What departments offered classes with 100 or more seats?

3.  What terms had the highest number of seats?
